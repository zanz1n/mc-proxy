...
use crate::{error::EncodeError, nbt::encode::write_compound_tag};
...
impl<T: EnumEncoder> Encoder for T {
    fn encode<W: Write>(&self, writer: &mut W) -> Result<(), EncodeError> {
        var_int::encode(&(self.get_type_id() as i32), writer)?;
...
impl<W: Write> EncoderWriteExt for W {
    ...
    fn write_string(&mut self, value: &str, max_length: u16) -> Result<(), EncodeError> {
        ...
        self.write_var_i32(length as i32)?;
...
pub mod rest {
    ...
    pub fn encode<W: std::io::Write>(value: &[u8], writer: &mut W) -> Result<(), EncodeError> {
...
pub mod bool_option {
    ...
    pub fn encode<W: Write, T: Encoder>(
        value: &Option<T>,
        writer: &mut W,
    ) -> Result<(), EncodeError> {
