...
impl Payload {
    ...
    pub fn translation(translate: &str, with: Vec<FormatedMessage>) -> Self {
        Payload::Translation {
...
impl Message {
    ...
    pub fn new(payload: Payload) -> Self {
        Self::Formated(FormatedMessage::new(payload))
    }

    #[inline]
    pub fn from_str(text: &str) -> Message {
        Self::Formated(FormatedMessage::new(Payload::text(text)))
    }
...
pub struct FormatedMessage {
    ...
    pub extra: Vec<FormatedMessage>,
}

impl FormatedMessage {
    #[inline]
    pub fn new(payload: Payload) -> Self {
        FormatedMessage {
            bold: None,
    ...
    #[inline]
    pub fn from_str(text: &str) -> FormatedMessage {
        FormatedMessage::new(Payload::text(text))
    }
...
pub struct MessageBuilder {
    current: FormatedMessage,
    root: Option<FormatedMessage>,
}
...
impl MessageBuilder {
    pub fn builder(payload: Payload) -> Self {
        let current = FormatedMessage::new(payload);
    ...
    pub fn then(mut self, payload: Payload) -> Self {
        ...
        self.current = FormatedMessage::new(payload);
        self
    }

    pub fn build(self) -> Message {
        Message::Formated(match self.root {
            Some(mut root) => {
                root.extra.push(self.current);
                root
            }
...
fn test_serialize_translate_opped_steve() {
    let with = vec![FormatedMessage::new(Payload::text("Steve"))];
    let message = FormatedMessage::new(Payload::translation("Opped %s", with));
...
fn test_deserialize_translate_opped_steve() {
    let with = vec![FormatedMessage::new(Payload::text("Steve"))];
    let expected_message = FormatedMessage::new(Payload::translation("Opped %s", with));

    assert_eq!(
        expected_message,
        FormatedMessage::from_json(include_str!("../../test/chat/translate_opped_steve.json"))
            .unwrap()
    );
}
